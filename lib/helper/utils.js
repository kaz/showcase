...
const getAddress = async app => {
	return (await inspect(app)).NetworkSettings.Networks[settings.domain].IPAddress;
};

const inspect = async app => {
	const stat = await __exec(`docker inspect ${app.name}`);
	return JSON.parse(stat)[0];
};
const dockerLogs = async app => {
	return await __exec(`docker logs --tail 512 ${app.name}`);
};

module.exports = {
	rimraf, unlink, readFile, writeFile, exec, question,
	request, renderConfig,
	getGateway, getAddress, inspect, dockerLogs,
};
