...
const getAddress = async app => {
	return JSON.parse(await __exec(`docker inspect ${app.name}`))[0].NetworkSettings.Networks[settings.domain].IPAddress;
};
const getStatus = async app => {
	const stat = await __exec(`docker inspect ${app.name}`).catch(_ => null);
	if(stat){
		return JSON.parse(stat)[0].State.Status;
	}
	return "error";
};
...
module.exports = {
	...
	getGateway, getAddress, getStatus, getLog,
};
