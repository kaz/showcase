...
const install = async app => {
	...
	if(cloneResult === null){
		...
		await utils.exec(`cd ${app.repoPath} && git fetch && git reset --hard origin/${app.document.branch} && git submodule update --recursive`);
	}
...
const setupNginx = async _ => {
	...
	await utils.renderConfig("system", {gateway}, `./data/nginx/conf.d/!.conf`);
