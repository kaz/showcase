...
const mariaIdentity = require("../identity/maria");
const mongoIdentity = require("../identity/mongo");

const setupDatabase = async (conn, repo, config) => {
	const identities = [
		mariaIdentity.new(conn),
		mongoIdentity.new(conn)
	];
...
const unsetNginx = async repo => {
	await utils.unlink(`./data/nginx/${utils.repo2name(repo)}.conf`);
};

module.exports = {setupDatabase, prepareDocker, setupNginx, unsetNginx};
