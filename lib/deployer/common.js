"use strict";

const ejs = require("ejs");
const path = require("path");

const utils = require("../utils");
const Logger = require("../logger");
const settings = require("../../settings");

const keyClass = [
	require("../key/maria"),
	require("../key/mongo"),
];

const setupDatabases = async (app, config) => {
	const dbEnv = {};
	
	for(const keyInst of keyClass.map(Clas => new Clas(app))){
		if(keyInst.needed(config)){
			if(!keyInst.key){
				await keyInst.create();
			}
			for(const objKey in keyInst.key){
				dbEnv[`${keyInst.type}_${objKey}`.toUpperCase()] = keyInst.key[objKey];
			}
		}
	}
	
	return dbEnv;
};
const unsetDatabases = async app => {
	for(const keyInst of keyClass.map(Clas => new Clas(app))){
		await keyInst.drop().catch(_ => 0);
	}
};

const __freezeEnv = env => {
	const opts = [];
	for(const key in env){
		opts.push(`--env ${key}="${env[key]}"`);
	}
	return opts;
};

const prepareDocker = async (app, config, dbEnv) => {
	// get repository path
	const repoPath = path.resolve(path.join("./data/repositories", app.document.repo));
	
	// determine docker options
	const options = __freezeEnv(dbEnv);
	options.push(`--name ${app.name}`);
	options.push(`--hostname ${app.name}`);
	options.push(`--volume ${repoPath}:/srv`);
	options.push(config.expose ? `--publish ${config.expose}:${config.expose}` : ``);
	options.push(`--network ${settings.domain}`);
	options.push(`--restart always`);
	options.push(`--workdir /srv`);
	options.push(config.image || `kazsw/arch`);
	
	if(config.on_created){
		// generate setup script
		const tmpFile = await utils.tmpName();
		await utils.writeFile(tmpFile, `#!/bin/bash -xe\n${config.on_created}`);
		
		// run setup script
		Logger.log("Running bootup script ...");
		await utils.exec(`docker rm -f ${app.name} || true`);
		await utils.exec(`docker create --entrypoint bash ${options.join(" ")} /root/setup.sh`);
		await utils.exec(`docker cp ${tmpFile} ${app.name}:/root/setup.sh`);
		await utils.exec(`docker start -i ${app.name}`);
		
		// delete setup script
		await utils.unlink(tmpFile);
	}
	
	// start container
	Logger.log("Running container ...");
	await utils.exec(`docker rm -f ${app.name} || true`);
	return async _ => await utils.exec(`docker run -dit ${options.join(" ")} ${config.entrypoint || ""}`);
};

const setupNginx = async (app, config) => {
	// computed config
	const computed = {
		config_file: `${settings.name}.yaml`
	};
	
	// determine default server name
	computed.servers = [`${app.name}.${settings.domain}`];
	
	// set additional server names
	const checkPush = cname => {
		if(new RegExp(`^.*\\.?${settings.domain}`).test(cname) || /^.*\.?trapti\.tech$/.test(cname)){
			throw new Error(`Cannot CNAME: ${cname}`);
		}
		computed.servers.push(cname);
	};
	if(typeof config.cname === "string"){
		checkPush(config.cname);
	}else if(Array.isArray(config.cname)){
		for(const cname of config.cname){
			checkPush(cname);
		}
	}
	
	// determine document root
	computed.document_root = path.join("/srv", app.document.repo, config.document_root || ".");
	computed.fastcgi_root = path.join("/srv", config.document_root || ".", "/");
	
	// find IP address
	if(config.type === "runtime" && config.http_proxy){
		computed.proxy_pass = (await utils.getAddress(app.name)) + ":" + config.http_proxy;
	}
	
	// generate config
	const configFile = `./data/nginx/${app.name}.conf`;
	const configStr = ejs.render(await utils.readFile("./resources/nginx.conf.ejs", "utf-8"), {config, computed});
	await utils.writeFile(configFile, configStr);
	
	// apply config
	Logger.log("Checking nginx config ...");
	await utils.exec("docker exec nginx nginx -t");
	
	Logger.log("Applying config ...");
	await utils.exec("docker exec nginx nginx -s reload");
};

const cancel = async app => {
	await utils.unlink(`./data/nginx/${app.name}.conf`).catch(_ => 0);
	await utils.exec(`docker rm -f ${app.name}`).catch(_ => 0);
};

module.exports = {setupDatabases, unsetDatabases, prepareDocker, setupNginx, cancel};
