...
const run = async (conn, repo) => {
	...
	const rawConfig = await utils.readFile(`${repoPath}/${pkg.name}.yaml`, "utf-8").catch(_ => null);
	if(!rawConfig){
...
const remove = async (conn, repo) => {
	...
	const app = mongo.db(pkg.name).collection("applications");
	const log = mongo.db(pkg.name).collection("logs");
...
const saveLog = async (conn, repo, log) => {
	const mongo = await conn.mongo();
	const logs = mongo.db(pkg.name).collection("logs");
