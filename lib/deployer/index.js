...
const run = async (conn, repo) => {
	...
	const cloneResult = await utils.exec(`git clone http://localhost:3000/${repo}.git ${repoPath}`).catch(_ => null);
	if(cloneResult === null){
		// pull repo
	...
	const rawConfig = await utils.readFile(`${repoPath}/traplus.yaml`, "utf-8").catch(_ => null);
	if(!rawConfig){
...
const cancel = async repo => {
	...
	await utils.unlink(`./data/nginx/${name}.conf`);
	await utils.exec(`docker rm -f ${name}`);
};
...
const remove = async (conn, repo) => {
	...
	const app = mongo.db("traplus").collection("applications");
	const log = mongo.db("traplus").collection("logs");
...
const saveLog = async (conn, repo, log) => {
	const mongo = await conn.mongo();
	const logs = mongo.db("traplus").collection("logs");
