...
class Deployer {
	...
	async docker(onlyStartup){
		...
		// create directory
		await utils.mkdir(path.join(this.app.repoPath, ".showcase")).catch(_ => 0);
		
		// run setup script
		if(config.startup){
			Logger.log("Running startup script ...");
			await utils.writeFile(path.join(this.app.repoPath, ".showcase/startup.sh"), config.startup || "");
			await utils.writeFile(path.join(this.app.repoPath, ".showcase/entrypoint.sh"), "read");
			await utils.exec(`docker start ${this.app.name}`);
			await utils.exec(`docker exec -i ${this.app.name} bash /srv/.showcase/startup.sh`);
			await utils.exec(`docker stop -t 0 ${this.app.name}`);
		}
		...
		if(onlyStartup){
			...
			await utils.writeFile(path.join(this.app.repoPath, ".showcase/entrypoint.sh"), config.entrypoint);
			await utils.exec(`docker start ${this.app.name}`);
		}
	}
	async nginx(){
