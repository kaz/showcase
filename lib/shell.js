...
const login = async user => {
	...
	if(select.repo == temporaryLabel){
		...
		if(status === "running"){
			console.log(`\na) Attach to main process`)
			console.log(`s) Launch shell`);
			
			if(/^a/i.test(await utils.question("\nSelect (a/s): "))){
	...
	const shell = child_process.spawn("docker", args, {stdio: "inherit"});
	const killer = child_process.spawn("node", [modulePath, killData], {detached: true, stdio: "ignore"});
	
	shell.on("exit", _ => killer.kill());
};
