...
module.exports.new = conn => {
	...
	const create = async repo => {
		...
		// save identities
		await mongo.db(settings.name).collection("identities").insertOne(get(repo));
	};
	const drop = async repo => {
		...
		// remove identities
		await mongo.db(settings.name).collection("identities").deleteMany({repo, type});
	};
	
	const resolve = async repo => {
		const mongo = await conn.mongo();
		const doc = await mongo.db(settings.name).collection("identities").findOne({repo, type});
		if(doc){
			database = doc.data;
			password = doc.pass;
			return true;
