...
class PubKey {
	...
	static async authorized_keys(){
		...
		return data.map(item => `command="${process.argv[0]} --harmony ${modulePath} ${item.user}",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty ${item.key}`).join("\n");
	}
